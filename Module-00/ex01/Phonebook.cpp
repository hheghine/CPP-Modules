...
/*   Updated: 2023/10/11 02:44:43 by hbalasan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Phonebook::add(void)
{
	...
	if (this->idx > 7)
		std::cout << \
			"Warning: overwriting the information about " \
			<< this->contacts[idx % 8].get_fname()  \
			<< std::endl;
	...
	while (!std::cin.eof() && str == "")
	{
		std::cout << "Enter " << this->contacts[idx % 8].get_fname() \
			<< "'s last name: ";
		if (std::getline(std::cin, str) && str != "")
	...
	while (!std::cin.eof() && str == "")
	{
		std::cout << "Enter " << this->contacts[this->idx % 8].get_fname() \
			<< "'s nickname: "; 
		if (std::getline(std::cin, str) && str != "")
	...
	while (!std::cin.eof() && str == "")
	{
		...
		if (std::getline(std::cin, str) && str != "")
		{
			...
			std::cout << this->contacts[this->idx % 8].get_fname() << \
				" successfully added to phonebook [" << this->idx % 8 + 1 << "/8]" << std::endl;
		}
	}
	this->idx++;
}
